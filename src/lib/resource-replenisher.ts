...
function buildReplenishPrompt(contentPreference?: ContentPreference): string {
  const prefClause = contentPreference === "video"
    ? "\n- 用户偏好视频类资料，请优先推荐 Bilibili、YouTube 等平台的 video 类型资源"
    : contentPreference === "article"
    ? "\n- 用户偏好文章类资料，请优先推荐博客、论文、文档等 article/paper 类型资源"
    : ""

  return `你是一个学习资料推荐助手。用户在学习计划中需要补充学习资料，请根据每天的学习主题推荐真实、高质量的资料。
...
- 推荐资源应为公开、合法的教育内容，禁止推荐盗版资源、付费破解内容或侵权材料
	- 推荐的学习资料必须是最新的（优先近 2-3 年内发布的），避免推荐过时或已失效的链接
	- groups 数组的第 N 个元素对应用户的第 N 组请求，每个 group 内的资源数量与用户请求的数量一致${prefClause}`
}
...
总共需要 ${contexts.reduce((s, c) => s + c.droppedCount, 0)} 个资料。`
  ...
  try {
    const response = await chat(
      [
        { role: "system", content: buildReplenishPrompt(contentPreference) + "\n\n**输出格式（严格按此 JSON，只输出 JSON，不要其他文字）：**\n" + REPLENISH_OUTPUT_FORMAT },
